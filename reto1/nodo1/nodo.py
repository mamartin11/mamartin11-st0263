...
class Node:
    ...
    def setup_routes(self):
        ...
        def joinNetwork(ip,port):
            self.sendJoinRequest(ip, int(port))
            return jsonify({"status": "success join"})

        @self.app.route('/leaveNetwork', methods=['GET'])
        def leaveNetwork():
            self.leaveNetwork()
            return jsonify({"status": "success leave"})

        @self.app.route('/uploadFile/<filename>', methods=['GET'])
        def uploadFile(filename):
            fileID = getHash(filename)
            recvIPport = self.getSuccessor(self.succ, fileID)
            self.uploadFile(filename, recvIPport, True)
            return jsonify({"status": "success upload"})

        @self.app.route('/downloadFile/<filename>', methods=['GET'])
        def downloadFile(filename):
            self.downloadFile(filename)
            return jsonify({"status": "success download"})
    
        @self.app.route('/printFTable', methods=['GET'])
        def printFTable():
            self.printFTable()
            return jsonify({"status": "success printFTable","fTable": self.fingerTable})
        
        @self.app.route('/printMyInfo', methods=['GET'])
        def printMyInfo():
            return jsonify({"status": "success printMyInfo", "id": self.id, "pred": self.predID, "succ": self.succID})
        
        @self.app.route('/printMyFiles', methods=['GET'])
        def printMyFiles():
            return jsonify({"status": "success printMyFiles", "files": self.filenameList})
